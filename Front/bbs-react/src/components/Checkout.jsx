...
export default function Checkout({ fechar, onComplete }) {
  ...
  const totais = calcularTotais(subtotal, entrega, pagamento);
  function campo(key, label, maxLength, minLength = 1) {
    return <label>{label}<input value={endereco[key]} maxLength={maxLength} minLength={minLength} required={key !== "complemento"}
      onChange={e => setEndereco({...endereco,[key]:key === "uf" ? e.target.value.toUpperCase() : e.target.value})} /></label>;
  }
  ...
  async function confirmar() {
    ...
    try {
      const result = await request("/pedidos", {method:"POST",body:{
        ...
        itens:cartOrder.filter(id => cart[id]).map(id => ({produtoId:Number(id),quantidade:cart[id].qty}))
      }});
  ...
  return <Modal title={pedido ? "Pedido registrado" : "Finalizar pedido"} fechar={() => {if(!busy) fechar();}}>
    ...
    cartOrder.length === 0 ? <p>Seu carrinho está vazio.</p> : <>
      ...
      {step === 2 && <form className="bbs-form" onSubmit={e => {e.preventDefault(); if(endereco.cep.replace(/\D/g,"").length!==8){setError("CEP deve ter 8 dígitos.");return;}setError("");setStep(3);}}>
        ...
        <label>Entrega demonstrativa<select value={entrega} onChange={e => setEntrega(e.target.value)}><option value="normal">Normal — R$ 15,90</option><option value="expresso">Expresso — R$ 29,90</option></select></label>
        <p>Valores fixos do projeto, sem cotação de transportadora.</p>
      ...
      {step === 3 && <div className="bbs-form">
        ...
        <p>Subtotal: {fmt(subtotal)} · Frete: {fmt(totais.frete)} · Desconto: {fmt(totais.desconto)}</p>
        <h3>Total estimado: {fmt(totais.total)}</h3><p>A API confirma os preços atuais e valida o estoque ao salvar.</p>
        <div className="bbs-actions"><button disabled={busy} onClick={() => setStep(2)}>Voltar</button><button disabled={busy} className="bbs-primary" onClick={confirmar}>{busy ? "Registrando…" : "Confirmar pedido demonstrativo"}</button></div>
      </div>}
    </>}
  </Modal>;
}
