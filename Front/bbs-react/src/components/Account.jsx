...
export default function Account({ usuario, onUsuario, fechar, abrirProdutos, onChange }) {
  ...
  return <Modal title={usuario ? (admin ? "Administração BBS" : "Minha conta") : (cadastro ? "Criar conta" : "Entrar na BBS")} fechar={fechar}>
    ...
    {!usuario ? <form className="bbs-form" onSubmit={entrar}>
      ...
      <div className="bbs-actions">
        <button onClick={() => setTab("pedidos")}>{admin ? "Pedidos e indicadores" : "Meus pedidos"}</button>
        <button onClick={() => setTab("perfil")}>Editar perfil</button>
        {admin && <><button onClick={() => setTab("clientes")}>Clientes e usuários</button><button onClick={abrirProdutos}>Gerenciar produtos</button></>}
        <button disabled={busy} onClick={sair}>Sair</button>
        <button disabled={busy || loading} onClick={() => {setLoading(true);setRevision(v => v+1);}}>Atualizar</button>
      </div>
      {tab === "perfil" ? <form className="bbs-form" onSubmit={perfil}>
      ...
      </form> : loading ? <p role="status">Carregando registros…</p> : tab === "clientes" ? <div className="bbs-table-wrap"><table className="bbs-table"><thead><tr><th>Nome</th><th>E-mail</th><th>Perfil</th></tr></thead><tbody>{clientes.map(c => <tr key={c.id}><td>{c.nome}</td><td>{c.email}</td><td>{c.perfil}</td></tr>)}</tbody></table></div> : <>
        ...
        {!pedidos.length && <p>Nenhum pedido registrado. As compras aparecerão aqui após a confirmação.</p>}
        {pedidos.map(p => <article className="bbs-order" key={p.id}>
          <div className="bbs-actions"><h3>Pedido #{p.id}</h3><span className="bbs-tag">{p.status}</span></div>
